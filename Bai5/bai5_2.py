# Nguyễn Duy Hiếu; Msv: 245752021610054
# Khai báo thư viện và lệnh format định dạng thời gian
import datetime as dt

format = '%Y-%m-%dT%H:%M:%S'
# Dùng hàm strptime để chuyển chuỗi ngày giờ thành đối tượng datetime
t1 = dt.datetime.strptime('2008-10-12T14:45:52', format)

print('Day: ' + str(t1.day))
print('Month: ' + str(t1.month))
print('Minute: ' + str(t1.minute))
print('Second: ' + str(t1.second))
# Nhập thời gian hiện tại và lệnh diff tính khoảng cách thời gian
t2 = dt.datetime.now()

diff = t2 - t1

print('How many days difference? ' + str(diff.days))
